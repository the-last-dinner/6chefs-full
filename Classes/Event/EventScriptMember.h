//
//  EventScriptMember.h
//  LastSupper
//
//  Created by Kohei Asami on 2015/10/24.
//
//

#ifndef __LastSupper__EventScriptMember__
#define __LastSupper__EventScriptMember__

namespace member
{
    static const char* ACTION = "action";
    static const char* ALPHA = "alpha";
    static const char* AMBIENT = "ambient";
    static const char* COLOR = "color";
    static const char* CONDITION = "condition";
    static const char* CHARA_ID = "charaID";
    static const char* CHAPTER_ID = "chapterID";
    static const char* CHASER = "chaser";
    static const char* CHOICE = "choice";
    static const char* CHOICES = "choices";
    static const char* DIRECTION = "direction";
    static const char* EASING = "easing";
    static const char* ENEMY_ID = "enemyID";
    static const char* EQUIP = "equip";
    static const char* EVENT = "event";
    static const char* EVENT_ID = "eventID";
    static const char* FALSE_ = "false";
    static const char* FAVORITE = "favorite";
    static const char* FLAG = "flg";
    static const char* FILE = "file";
    static const char* ID = "id";
    static const char* IMGS = "imgs";
    static const char* IMG_ID = "imgID";
    static const char* INFO_ID = "infoID";
    static const char* ITEM_ID = "itemID";
    static const char* LAYER = "layer";
    static const char* LIMIT = "limit";
    static const char* MAP_ID = "mapID";
    static const char* MOVE_PATTERN = "movePattern";
    static const char* NAME = "name";
    static const char* NEXT_X = "nextX";
    static const char* NEXT_Y = "nextY";
    static const char* OBJECT_ID = "objID";
    static const char* OPTION = "option";
    static const char* PASSWORD = "pass";
    static const char* SECOND = "second";
    static const char* SPEED = "speed";
    static const char* STATUS = "status";
    static const char* STEPS = "steps";
    static const char* TALK = "talk";
    static const char* TARGET = "target";
    static const char* TEXT = "text";
    static const char* TIME = "time";
    static const char* TITLE = "title";
    static const char* TIMES = "times";
    static const char* TRUE_ = "true";
    static const char* TYPE = "type";
    static const char* VOLUME = "volume";
    static const char* X = "x";
    static const char* Y = "y";
}

#endif /* defined(__LastSupper__EventScriptMember__) */
