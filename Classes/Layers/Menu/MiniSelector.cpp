...
bool MiniSelector::init(Selector& selector)
{
    ...
    const Size panelSize = Size(backSize.width/index.x, (backSize.height - verticalMargin * 2) /index.y);
    ...
    for(int i = 0; i < menu_size; i++)
    {
        ...
        panel->setTextureRect(Rect(0,0,panelSize.width, panelSize.height));
        panel->setPosition(((i%(int)index.x)) * panelSize.width + panelSize.width/2, backSize.height - (floor(i/(int)index.x) + 1) * panelSize.height + panelSize.height/2 - verticalMargin);
        panel->setOpacity(0);
