...
bool DungeonMainMenuLayer::init()
{
	...
	for(int i {0}; i < static_cast<int>(Type::SIZE); i++)
	{
		...
		menu->setPosition((WINDOW_WIDTH / static_cast<int>(Type::SIZE)) * (i + 1), 40);
		hBg->addChild(menu);
		this->menuObjects.push_back(menu);
	}
	
	// マップ名表示
	Label* mapName = Label::createWithTTF("マップ名", "fonts/cinecaption2.28.ttf", 26);
	hBg->setPosition(mapName->getContentSize().width / 2, hBg->getContentSize().height - mapName->getContentSize().height / 2);
