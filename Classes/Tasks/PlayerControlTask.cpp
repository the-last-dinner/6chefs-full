...
void PlayerControlTask::walking(const vector<Key>& keys)
{
    ...
    float ratio {this->listener->isPressed(Key::DASH)? 2.f : 1.f};
    
    // 入力確認の頻度を変更
    this->listener->setInputCheckInterval(Character::DURATION_FOR_ONE_STEP / ratio);
    ...
    this->party->move(movement, ratio);
}
...
void PlayerControlTask::onCharacterWalkedOneGrid()
{
    ...
    Rect collisionRect {this->party->getMainCharacter()->getCollisionRect()};
